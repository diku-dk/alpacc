...
using token_t = unsigned char;
using state_t = unsigned short;

const unsigned int NUM_STATES = 12;
...
void testLexer(unsigned char* input,
               size_t input_size,
               unsigned int* expected_indices,
    ...
    const I IN_ARRAY_BYTES = size * sizeof(unsigned char);
    ...
    gpuAssert(cudaMalloc((void**)&d_dyn_index_ptr, sizeof(unsigned int)));
    gpuAssert(cudaMalloc((void**)&d_new_size, sizeof(I)));
    gpuAssert(cudaMalloc((void**)&d_is_valid, sizeof(bool)));
    cudaMemset(d_dyn_index_ptr, 0, sizeof(unsigned int));
    ...
    I padding = IN_ARRAY_BYTES; // sizeof(unsigned long long) - (IN_ARRAY_BYTES % sizeof(unsigned long long));
